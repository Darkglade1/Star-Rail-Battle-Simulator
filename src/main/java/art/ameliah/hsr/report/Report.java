...
public class Report {
    ...
    public void generateCSV() {
        ...
        for (EnemyTeam enemyTeam : enemyTeams) {
            ...
            baselineDPAV = battle.getTotalPlayerDmg() / battle.getActionValueUsed();
            ...
            for (PlayerTeam playerTeam : otherTeams) {
                ...
                float otherTeamDPAV = battle.getTotalPlayerDmg() / battle.getActionValueUsed();
    ...
    public void updateCharacterCSVs(Battle battle, HashMap<String, HashMap<String, ArrayList<String>>> characterMetricsMap, HashMap<String, ArrayList<String>> characterTeamList, HashMap<String, ArrayList<String>> characterMetricOrderList, HashMap<String, ArrayList<String>> damageContributionMap, HashMap<String, String> characterStatsMap, ArrayList<AbstractCharacter<?>> team, PlayerTeam playerTeam) {
        for (AbstractCharacter<?> character : team) {
            ...
            if (!characterTeamList.containsKey(character.getName())) {
                ArrayList<String> teamList = new ArrayList<>();
                characterTeamList.put(character.getName(), teamList);
            }
            ArrayList<String> list = characterTeamList.get(character.getName());
            list.add(playerTeam.toString());

            if (!damageContributionMap.containsKey(character.getName())) {
                ArrayList<String> damageContributionList = new ArrayList<>();
                damageContributionMap.put(character.getName(), damageContributionList);
            }
            ArrayList<String> damageList = damageContributionMap.get(character.getName());
            float damagePercent = battle.damageContributionMapPercent.get(character);
            float rawDamage = battle.dmgContributionMetric.map.get(character);
            float DPAV = rawDamage / battle.initialLength();
            damageList.add(String.format("%.2f DPAV (%.1f%% of total)", DPAV, damagePercent));

            characterMetricOrderList.put(character.getName(), character.getOrderedCharacterSpecificMetricsKeys());
